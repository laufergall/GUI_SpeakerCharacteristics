...
public class ListSpeech {
	...
	private void removeKnob() {
		...
		for (int i=0; i<interf.allSliders.size(); i++){
			...
			if(interf.allRatings[i]==midVal){
				interf.allSliders.get(i).setValue(midVal+1); 
			}
			// Reset slider value
			interf.allSliders.get(i).setValue(midVal);

			// Remove knob and do not allow to paint it
			interf.allSliders.get(i).knobEnabled=false;
			interf.allSliders.get(i).knobPainted=false;
		}
